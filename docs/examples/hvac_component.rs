...
impl FaultSink for VehicleBusSink {
    ...
    async fn publish(&self, record: fault_lib::model::FaultRecord) -> Result<(), SinkError> {
        println!(
...
async fn handle_blower_fault(mut reporter: Reporter, measured_rpm: f32, commanded_rpm: f32) {
    ...
    if let Err(err) = reporter.report(descriptor, options).await {
